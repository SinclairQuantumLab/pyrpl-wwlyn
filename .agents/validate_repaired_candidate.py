"""Agent-only offline/package validation of the repaired Z7020 test candidate."""
...
def validate(repo, output, base):
    ...
    try:
        ...
        run(['git', 'diff', '--exit-code', 'e747916', '--', 'pyrpl/fpga/rtl'],
        ...
        cases = ['tests.test_python314_compatibility', 'tests.test_ipykernel_compatibility',
                 ...
                 'tests.test_z7020_repaired_baseline', 'tests.test_z7020_repaired_loader']
        ...
        run(['uv', 'run', '--extra', 'test', 'nosetests',
             'pyrpl.test.test_memory', 'pyrpl.test.test_proxyproperty'], 'nose')
        results['rtl_simulation'] = 'PASS separately: real XSim, 342 PID checks and five x 4096 filter checks; TEMP/pyrpl-pid-rtl-nmlkz1w7'
        ...
        with zipfile.ZipFile(wheel) as archive:
            ...
            assert hashlib.sha256(archive.read('pyrpl/fpga/red_pitaya_z20_gen2_author.bit.bin')).hexdigest() == (
                'f6728daaf863f6c48a1d8b27a7262d7fb7653c489f37db36acd6cbb9cc4a7307')
            candidate = 'pyrpl/fpga/red_pitaya_z20_gen2_repaired.bit.bin'
            record = json.loads(archive.read('pyrpl/fpga/red_pitaya_z20_gen2_repaired.json'))
            assert hashlib.sha256(archive.read(candidate)).hexdigest() == record['bitstream_sha256']
            assert record['common_pid_filter_fixes_included'] is True
