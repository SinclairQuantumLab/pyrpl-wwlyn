...
code sets `asg.frequency = 1` and amplitude 0.4. This API uses Hz. The user subsequently clarified that **both 1 Hz and 1 kHz were tested and
worked well**. The saved trace still documents the 1 Hz setting; the 1 kHz
success is an additional user-reported observation. The repeat includes both
frequencies. The old aliasing explanation cannot be attributed to the retained
1 Hz run. Mean/midrange offsets are observations,
...
Retain the same DC levels, triangle amplitude 0.4 at both 1 Hz and 1 kHz,
and negative-I feedback test (restore the saved 1 Hz disturbance for PID). Capture fresh paired DC readings and fit the Pro board independently;
...
This copy follows the initial Gen 1 documentation commit `8c6f4df` and includes
the user's subsequent two-frequency clarification while preparing the Pro run.
