...
On 2026-09-24 the user requested promotion of the successfully device-tested
pre-repair baselines, then clarified that standard Gen2 Z7010 had NOT been
tested because no such device is available. Acceptance covers Gen1 OS1,
Gen1 OS2 and Gen2 Pro OS2 only, before adopting the shared PID/filter repairs.
The earlier interpretation of "all" as four combinations was an agent error;
the mistakenly created local `gen2-os2/main` is withdrawn. Standard Gen2
development/candidate branches remain untested, with the field gate open.
This is user-reported field acceptance, not a new
...
| `gen2pro-os2/main` | `7cb67e263c3643cb09f2580fcf666502381ffcf7` | Original-logic Pro device-upgrade candidate, not the older partially repaired Pro develop. |

All three selected RTL trees match author checkpoint
...
new measurements. The later Pro functional success is recorded here as the
user's confirmation. No standard Gen2 Z7010 hardware success is claimed.

The user also requested renaming the Pro branch prefix from `gen2-pro-os2/`
to `gen2pro-os2/`, locally and remotely. Historical records may retain the
old spelling; the rename does not change device support or FPGA artifacts.
