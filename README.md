...
### Repaired Gen1 / OS2 bench candidate

`gen1-os2/develop` includes the common PID source repairs in the separate
...
This is an original STEMlab 125-14 Z7010 / OS2.07+ candidate, not a Gen2 or
Z7020 image. Existing firmware-name DTBOs are retained. Timing findings remain
...
Use `test.ipynb` for your settings and results; it is no longer ignored by
Git on this branch. If it is absent, copy `test.ipynb.template` to create it
and edit its hostname. Do not replace an existing notebook with the template.
